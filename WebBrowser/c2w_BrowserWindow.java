...
class c2w_BrowserWindow extends Stage{
    ...
    c2w_BrowserWindow(c2w_WebBrowser c2w_browser,String c2w_initialURL){
        ...
        c2w_webEngine.getLoadWorker().stateProperty().addListener((o,oldVal,newVal) -> {
            ...
            switch(newVal){
                ...
                case SUCCEEDED:
                    ...
                    break;
                case FAILED:
                    status.setText("Status: Loading of the web page has been failed.");
                    break;
                case CANCELLED:
        ...
        c2w_webEngine.setPromptHandler(promptData -> 
                            SimpleDialogs.prompt(promptData.getMessage(), "Query from web page", promptData.getDefaultValue()));
    ...
    private MenuBar makeMenuBar(){
        ...
        MenuItem newWin = new MenuItem("New windows");
        newWin.setOnAction( e -> c2w_owner.c2w_newBrowserWindow(null));
        MenuItem close = new MenuItem("Close Windows");
        close.setOnAction(e -> hide());
        MenuItem open = new MenuItem("Open Url in new Windows");
        open.setOnAction(e -> {
            String url = SimpleDialogs.prompt("Enter the url of teh page you want to load","get URL" );
            if(url != null && url.trim().length() >0)
